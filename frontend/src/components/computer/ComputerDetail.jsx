...
import { useFormatting, useModalState } from "../../app/index";
...
const ComputerDetail = ({
  ...
  // Redux state
  const errors = useAppSelector(selectComputerErrors);
  const errorsLoading = useAppSelector(selectComputerLoading);
  const fetchError = useAppSelector(selectComputerError);

  // Error management states
  const [actionLoading, setActionLoading] = useState(false);
  const [viewingError, setViewingError] = useState(null);

  const { isModalVisible: resolveModalVisible, selectedItem: currentError, openModal: openResolveModal, closeModal: closeResolveModal } = useModalState();
  ...
  // Load errors on component mount and when computer changes
  useEffect(() => {
    if (computer?.id) {
      dispatch(fetchComputerErrors(computer.id));
    }
  }, [dispatch, computer?.id]);

  const handleDelete = async () => {
    if (!computer?.id) return;
    try {
      await dispatch(deleteComputer(computer.id)).unwrap();
  ...
  const handleResolveSubmit = async () => {
    try {
      ...
      await dispatch(resolveComputerError({
        computerId: computer.id,
        errorId: currentError.id,
        resolutionNotes: values.resolution_notes
      })).unwrap();
  ...
  const handleReportSubmit = async () => {
    try {
      ...
      await dispatch(reportComputerError({
        computerId: computer.id,
  ...
  return (
    <div className="computer-detail">
        ...
        <div style={{ padding: '20px' }}>
          ...
          <Row align="middle" gutter={[8, 8]}>
            <Col span={24}>
              <Space>
                <InfoCircleOutlined />
                <Text type="secondary">Agent ID: {computer.agent_id || 'Not registered'}</Text>
              </Space>
            </Col>
          </Row>
        </div>
      ...
      <div className="computer-errors" style={{ padding: '20px', background: '#f0f2f5', borderRadius: '8px' }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: 'center', marginBottom: 16 }}>
          ...
          <div>
            ...
            <Button icon={<ReloadOutlined />} onClick={() => dispatch(fetchComputerErrors(computer.id))} disabled={errorsLoading || actionLoading}>
              Refresh
            </Button>
          </div>
        </div>
      ...
      <Modal
        ...
        onCancel={closeErrorDetailsModal}
        footer={[<Button key="back" onClick={closeErrorDetailsModal}>Close</Button>]}
      >
        {viewingError && (
          <>
            <Paragraph>
      ...
      <Modal
        ...
        onCancel={closeResolutionNotesModal}
        footer={[<Button key="back" onClick={closeResolutionNotesModal}>Close</Button>]}
      >
        {viewingError && (
          <>
            <Paragraph>
