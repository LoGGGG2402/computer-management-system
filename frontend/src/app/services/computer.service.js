...
class ComputerService {
   ...
   *     - total_ram {number} - Total RAM in bytes
   *     - total_disk_space {number} - Total disk space in bytes
  ...
  async getComputerErrors(id) {
    try {
      ...
      return response.data.status === 'success' ? response.data.data.errors : [];
    } catch (error) {
      // Nếu lỗi 404 "No errors found for this computer", trả về mảng rỗng
      if (error.response && error.response.status === 404 && 
          error.response.data.message === 'No errors found for this computer') {
        return [];
      }
