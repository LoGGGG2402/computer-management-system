...
sequenceDiagram
    ...
    participant AuthMiddleware as Auth Middleware (authUser.js)
    participant AuthController as Auth Controller
    participant AuthService as Auth Service
    participant UserDB as User Database (Sequelize)
    participant RefreshTokenDB as Refresh Token Database (Sequelize)

    FrontendUser->>FE: Enter Username & Password
    FE->>BE_AuthRoutes: POST /api/auth/login (username, password)
    ...
    UserDB-->>AuthService: Return user information (including password_hash)
    alt User exists and is active
        AuthService->>AuthService: Compare passwords (bcrypt.compare)
        alt Valid password
            AuthService->>AuthService: Create Access Token JWT (id, username, role)
            AuthService->>AuthService: Create Refresh Token (userId)
            AuthService->>RefreshTokenDB: Store hashed refresh token
            RefreshTokenDB-->>AuthService: Confirm token storage
            AuthService-->>AuthController: Return {id, username, role, accessToken, refreshToken, expires_at}
            AuthController-->>BE_AuthRoutes: Response 200 OK (userData, accessToken, set refreshToken as HttpOnly cookie)
            BE_AuthRoutes-->>FE: {id, username, role, accessToken, expires_at} + Set HttpOnly cookie with refreshToken
            FE-->>FrontendUser: Display successful login, save accessToken in memory
        else Invalid password
            AuthService-->>AuthController: Throw error "Invalid password"
            AuthController-->>BE_AuthRoutes: Response 401 Unauthorized
            BE_AuthRoutes-->>FE: Error "Invalid credentials"
            FE-->>FrontendUser: Display login error
        end
    else User doesn't exist or isn't active
        AuthService-->>AuthController: Throw error "User not found or inactive"
        AuthController-->>BE_AuthRoutes: Response 401 Unauthorized
        BE_AuthRoutes-->>FE: Error "Invalid credentials"
        FE-->>FrontendUser: Display login error
    end
    ...
    FE->>BE_AuthRoutes: GET /api/auth/me
    BE_AuthRoutes->>AuthMiddleware: verifyToken(req, res, next)
    AuthMiddleware->>AuthMiddleware: Verify JWT from header
    alt Valid token
        AuthMiddleware->>BE_AuthRoutes: next() (attach req.user)
        BE_AuthRoutes->>AuthController: handleGetMe(req, res)
        AuthController->>AuthService: getUserById(req.user.id)
        AuthService->>UserDB: Find user by ID (excluding password_hash)
        UserDB-->>AuthService: Return user data
        AuthService-->>AuthController: user data
        AuthController-->>BE_AuthRoutes: Response 200 OK (user data)
        BE_AuthRoutes-->>FE: User data
    else Invalid token
        AuthMiddleware-->>BE_AuthRoutes: Response 401/403
        BE_AuthRoutes-->>FE: Authentication error
    end
```
...
```mermaid
sequenceDiagram
    actor AgentInstaller as Agent Installer
    ...
    participant AdminFE as Admin Frontend (via WebSocket)
    participant ComputerDB as Computer Database
    participant RoomDB as Room Database
    participant MFACache as MFA Cache (NodeCache)

    AgentApp->>BE_AgentRoutes: POST /api/agent/identify (agentId, positionInfo: {roomName, posX, posY})
    BE_AgentRoutes->>AgentController: handleIdentifyRequest(req, res)
    AgentController->>ComputerService: findComputerByAgentId(agentId)
    ComputerService->>ComputerDB: Find computer by agentId
    alt Agent already exists and has agent_token_hash
        ComputerDB-->>ComputerService: Return computer
        ComputerService-->>AgentController: Return computer
        AgentController-->>BE_AgentRoutes: Response 200 OK {status: "success"}
        BE_AgentRoutes-->>AgentApp: {status: "success"} (Agent can use old token to connect WS)
    else New agent or has no token
        ComputerDB-->>ComputerService: null
        ComputerService-->>AgentController: null
        AgentController->>RoomService: isPositionAvailable(roomName, posX, posY)
        RoomService->>RoomDB: Find room by roomName, check layout and existing computers in room
        alt Invalid position (room doesn't exist, outside layout, already occupied)
            RoomDB-->>RoomService: Room information/error
            RoomService-->>AgentController: {valid: false, message: "Position error"}
            AgentController-->>BE_AgentRoutes: Response 400 Bad Request {status: "position_error", message}
            BE_AgentRoutes-->>AgentApp: Position error
        else Valid position
            RoomDB-->>RoomService: Room information (room.id, room.name)
            RoomService-->>AgentController: {valid: true, room: {id, name, layout}}
            AgentController->>MfaService: generateAndStoreMfa(agentId, {roomId: room.id, posX, posY})
            MfaService->>MFACache: Store mfaCode, positionInfo (including roomId) for agentId (with timeout)
            MfaService-->>AgentController: Return mfaCode
            AgentController->>WebSocketService: notifyAdminsNewMfa(mfaCode, {roomName: room.name, posX, posY, roomId: room.id})
            WebSocketService-->>AdminFE: Event 'admin:new_agent_mfa' (mfaCode, positionInfo)
            AdminFE-->>AgentInstaller: Display MFA code and position to installer
            AgentController-->>BE_AgentRoutes: Response 200 OK {status: "mfa_required"}
            BE_AgentRoutes-->>AgentApp: {status: "mfa_required"}
        end
    end

    AgentInstaller->>AgentApp: Enter MFA code
    AgentApp->>BE_AgentRoutes: POST /api/agent/verify-mfa (agentId, mfaCode)
    BE_AgentRoutes->>AgentController: handleVerifyMfa(req, res)
    AgentController->>MfaService: verifyMfa(agentId, mfaCode)
    MfaService->>MFACache: Check mfaCode with agentId, if valid remove from cache
    alt Valid MFA
        MfaService-->>AgentController: {valid: true, positionInfo: {roomId, posX, posY}}
        AgentController->>ComputerService: generateAndAssignAgentToken(agentId, positionInfo)
        ComputerService->>ComputerDB: Create new Computer record (agent_id, agent_token_hash, name (auto-generated), status='offline', room_id, pos_x, pos_y)
        ComputerDB-->>ComputerService: Return newly created computer object
        ComputerService-->>AgentController: {plainToken, computer}
        AgentController->>WebSocketService: notifyAdminsAgentRegistered(computer.id, positionInfo)
        WebSocketService-->>AdminFE: Event 'admin:agent_registered' (computerId, positionInfo)
        AgentController-->>BE_AgentRoutes: Response 200 OK {status: "success", agentToken: plainToken}
        BE_AgentRoutes-->>AgentApp: Save agentToken, registration successful
    else Invalid MFA (wrong code, expired, agentId mismatch)
        MfaService-->>AgentController: {valid: false, positionInfo: null}
        AgentController-->>BE_AgentRoutes: Response 401 Unauthorized {status: "error", message: "Invalid or expired MFA code"}
        BE_AgentRoutes-->>AgentApp: MFA error
    end
```
...
```mermaid
sequenceDiagram
    participant Client as Client (Frontend UI / Agent App)
    participant BE_SocketIO as Backend Socket.IO Server
    participant SocketIndexMiddleware as sockets/index.js Middleware
    participant FrontendHandler as Frontend WS Handler
    participant AgentHandler as Agent WS Handler
    participant WebSocketService as WebSocket Service

    Client->>BE_SocketIO: Request WebSocket connection
    note right of Client: Send headers 'X-Client-Type' ('frontend' or 'agent'), 'Authorization: Bearer <token>', (if agent: 'X-Agent-ID')
    BE_SocketIO->>SocketIndexMiddleware: io.use((socket, next) => { ... })
    SocketIndexMiddleware->>SocketIndexMiddleware: Extract clientType, authToken, agentId from headers, attach to socket.data
    SocketIndexMiddleware->>BE_SocketIO: next()
    BE_SocketIO->>SocketIndexMiddleware: io.on('connection', (socket) => { ... })
    alt socket.data.type === 'frontend'
        SocketIndexMiddleware->>FrontendHandler: setupFrontendHandlers(socket)
        FrontendHandler->>FrontendHandler: handleFrontendAuthentication(socket, {authToken: socket.data.authToken})
        FrontendHandler->>AuthService: (Indirectly via JWT verify) Verify authToken
        alt Valid token
            FrontendHandler->>WebSocketService: joinUserRoom(socket, decoded.id)
            alt User is Admin
                FrontendHandler->>WebSocketService: joinAdminRoom(socket)
            end
            FrontendHandler->>Client: Emit 'auth_response' {status: 'success', userId, role}
        else Invalid token
            FrontendHandler->>Client: Emit 'auth_response' {status: 'error', message}
            FrontendHandler->>Client: socket.disconnect()
        end
    else socket.data.type === 'agent'
        SocketIndexMiddleware->>AgentHandler: setupAgentHandlers(socket)
        AgentHandler->>AgentHandler: handleAgentAuthentication(socket, {agentId: socket.data.agentId, authToken: socket.data.authToken})
        AgentHandler->>ComputerService: verifyAgentToken(agentId, authToken)
        alt Valid token
            AgentHandler->>WebSocketService: joinAgentRoom(socket, computerId)
            AgentHandler->>Client: Emit 'agent:ws_auth_success' {status: 'success'}
            note right of Client: Agent begins sending 'agent:status_update'
        else Invalid token
            AgentHandler->>Client: Emit 'agent:ws_auth_failed' {status: 'error', message}
            AgentHandler->>Client: socket.disconnect()
        end
    else Undefined ClientType
        SocketIndexMiddleware->>Client: socket.disconnect(true)
    end
```
...
sequenceDiagram
    ...
    participant AgentHandler as Agent WS Handler
    participant WebSocketService as WebSocket Service
    participant FE_Subscribers as Frontend UI (Subscribed clients)

    loop Periodically or on status change
        AgentApp->>AgentApp: Collect information (CPU, RAM, Disk usage)
        AgentApp->>BE_SocketIO: Emit 'agent:status_update' ({cpuUsage, ramUsage, diskUsage})
        note right of AgentApp: Socket is authenticated (Agent Token), computerId is in socket.data
        BE_SocketIO->>AgentHandler: handleAgentStatusUpdate(socket, data)
        AgentHandler->>WebSocketService: updateRealtimeCache(socket.data.computerId, data) (store in Map agentRealtimeStatus)
        AgentHandler->>WebSocketService: broadcastStatusUpdate(socket.data.computerId)
        WebSocketService->>FE_Subscribers: Emit 'computer:status_updated' ({computerId, status:'online', cpuUsage, ramUsage, diskUsage, timestamp}) to room computer_<computerId>_subscribers
        FE_Subscribers->>FE_Subscribers: Update UI with new status information
    end
```
