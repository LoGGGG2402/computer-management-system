...
async function handleFrontendAuthentication(socket, data) {
  try {
    ...
    if (!authToken) {
      logger.warn(
        `Frontend authentication attempt with no token from ${socket.id}`
      );
    ...
    jwt.verify(authToken, config.accessToken.secret, (err, decoded) => {
      if (err) {
        const errorMessage =
        ...
        logger.warn(
          `Token authentication attempt failed from ${socket.id}: ${err.message}`
        );
      ...
      logger.info(
        `Frontend user authenticated: User ID ${decoded.id} (Role: ${decoded.role}), Socket ID ${socket.id}`
      );
    });
  } catch (error) {
...
async function handleComputerSubscription(socket, data) {
  ...
  const computerIdError = validationUtils.validatePositiveIntegerId(
    computerId,
    "Computer ID"
  );
  if (computerIdError) {
    logger.warn(
      `Invalid subscription attempt from ${socket.id}: ${computerIdError}`
    );
    socket.emit("subscribe_response", {
      status: "error",
      message: computerIdError,
    });
    return;
  }
  ...
  try {
    ...
    if (!hasAccess) {
      hasAccess = await computerService.checkUserComputerAccess(
        userId,
        computerId
      );
    }
    ...
    if (hasAccess) {
      ...
      logger.warn(
        `User ${userId} (Socket ${socket.id}) denied subscription to computer ${computerId}`
      );
    }
  } catch (error) {
    logger.error(
      `Subscription error for user ${userId}, computer ${computerId} (Socket ${socket.id}):`,
      {
        error: error.message,
        stack: error.stack,
      }
...
function handleComputerUnsubscription(socket, data) {
  ...
  if (computerIdError) {
    logger.warn(
      `Invalid unsubscribe attempt from ${socket.id}: ${computerIdError}`
    );
  ...
  try {
    const roomName =
      websocketService.ROOM_PREFIXES.COMPUTER_SUBSCRIBERS(computerId);
    socket.leave(roomName);
    socket.emit("unsubscribe_response", { status: "success", computerId });
    logger.info(
      `Client ${socket.id} unsubscribed from computer ${computerId} (Room: ${roomName})`
    );
  } catch (error) {
    logger.error(
      `Unsubscription error for computer ${computerId} (Socket ${socket.id}):`,
      {
        error: error.message,
        stack: error.stack,
      }
...
async function handleCommandSend(socket, data, ack) {
  ...
  if (typeof ack !== "function") {
    logger.warn(
      `frontend:send_command received without acknowledgement callback from socket ${socket.id}`
    );
    return;
  }
  ...
  if (!userId) {
    ...
    ack({ status: "error", message: "Not authenticated" });
    return;
  }
  ...
  if (computerIdError) {
    logger.warn(
      `Invalid command attempt from user ${userId} (Socket ${socket.id}): ${computerIdError}`
    );
    ack({ status: "error", message: computerIdError });
    return;
  }

  if (!command || typeof command !== "string") {
    logger.warn(
      `Invalid command attempt from user ${userId} (Socket ${socket.id}): Missing or invalid command type`
    );
    ack({ status: "error", message: "Command must be a string", computerId });
    return;
  }

  if (command.trim() === "" || command.length > 2000) {
    logger.warn(
      `Invalid command attempt from user ${userId} (Socket ${socket.id}): Empty or too long command (max 2000 chars)`
    );
    ack({
      status: "error",
      message: "Command must be non-empty and maximum 2000 characters",
      computerId,
    });
    return;
  }
  ...
  if (!validCommandTypes.includes(commandType)) {
    logger.warn(
      `Invalid command type "${commandType}" from user ${userId} (Socket ${socket.id})`
    );
    ack({
      status: "error",
      message: `Invalid command type. Must be one of: ${validCommandTypes.join(
        ", "
      )}`,
      computerId,
    });
  ...
  try {
    ...
    if (!hasAccess) {
      logger.warn(
        `Command access denied for user ${userId} to computer ${computerId} (Socket ${socket.id})`
      );
      ack({
        status: "error",
    ...
    const sent = websocketService.sendCommandToAgent(
      computerId,
      command,
      commandId,
      commandType
    ...
    if (sent) {
      logger.info(
        `Command ${commandId} (type: ${commandType}) initiated by user ${userId} sent towards computer ${computerId} (Socket ${socket.id})`
      );
      ack({ status: "success", computerId, commandId, commandType });
    } else {
      logger.warn(
        `Failed to send command ${commandId} (type: ${commandType}) from user ${userId} to computer ${computerId}: Agent not connected (Socket ${socket.id})`
      );
    ...
    logger.error(
      `Send command error for user ${userId}, computer ${computerId} (Socket ${socket.id}):`,
      {
        error: error.message,
        stack: error.stack,
      }
...
const setupFrontendHandlers = (socket) => {
  ...
  if (clientType !== "frontend") {
    logger.warn(
      `Invalid client type ${clientType} for socket ${socket.id}. Disconnecting.`
    );
    socket.emit("auth_error", {
  ...
  if (authToken) {
    logger.info(
      `Attempting auto-authentication for frontend client via headers: Socket ID ${socket.id}`
    );
    handleFrontendAuthentication(socket, { authToken });
  } else {
    logger.warn(
      `No auth token provided for socket ${socket.id}. Disconnecting.`
    );
    socket.emit("auth_error", {
      message: "Authentication failed: Missing Authorization header",
    });
...
const handleFrontendDisconnect = (socket) => {
  logger.info(
    `Frontend client disconnected: User ID ${
      socket.data.userId || "N/A"
    }, Socket ID ${socket.id}`
  );
};
