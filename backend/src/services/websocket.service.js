...
class WebSocketService {
  ...
  /**
   * Called when an agent socket successfully authenticates and identifies itself.
   * Joins the socket to the necessary rooms.
   * @param {import("socket.io").Socket} socket - The authenticated socket instance.
   * @param {number} computerId - The computer ID the agent represents.
   */
  setupAgentConnection(socket, computerId) {
    if (!socket || !computerId) {
      logger.error('setupAgentConnection called with invalid socket or computerId');
      return;
    }
    try {
      ...
      logger.info(`Socket ${socket.id} for Computer ${computerId} joined agent room: ${agentRoom}`);

    } catch (error) {
      logger.error(`Error setting up connection for agent ${computerId} (Socket ${socket.id}): ${error.message}`, { stack: error.stack });
      socket.disconnect(true);
    }
  }

  /**
   * Called when a client (e.g., admin UI) wants to subscribe to updates for a specific computer.
   * Joins the client's socket to the computer's subscriber room.
   * @param {import("socket.io").Socket} socket - The client's socket instance.
   * @param {number} computerId - The computer ID to subscribe to.
   */
  subscribeToComputerUpdates(socket, computerId) {
    if (!socket || !computerId) {
      logger.error('subscribeToComputerUpdates called with invalid socket or computerId');
      return;
    }
    try {
      const roomName = ROOM_PREFIXES.COMPUTER_SUBSCRIBERS(computerId);
      socket.join(roomName);
      logger.info(`Client socket ${socket.id} subscribed to updates for computer ${computerId} (joined room ${roomName})`);

      const currentStatus = this.getAgentRealtimeStatus(computerId);
      const isConnected = this.isAgentConnected(computerId);
      const statusPayload = {
        computerId,
        status: isConnected ? 'online' : 'offline',
        ...(currentStatus || { cpuUsage: 0, ramUsage: 0, diskUsage: 0 }),
        timestamp: currentStatus?.lastUpdated || new Date(),
      };
      socket.emit(EVENTS.COMPUTER_STATUS_UPDATED, statusPayload);

    } catch (error) {
      logger.error(`Error subscribing client ${socket.id} to computer ${computerId}: ${error.message}`, { stack: error.stack });
    }
  }

  /**
   * Called when a client wants to unsubscribe from updates for a specific computer.
   * Removes the client's socket from the computer's subscriber room.
   * @param {import("socket.io").Socket} socket - The client's socket instance.
   * @param {number} computerId - The computer ID to unsubscribe from.
   */
  unsubscribeFromComputerUpdates(socket, computerId) {
     if (!socket || !computerId) {
      logger.error('unsubscribeFromComputerUpdates called with invalid socket or computerId');
      return;
    }
     try {
      const roomName = ROOM_PREFIXES.COMPUTER_SUBSCRIBERS(computerId);
      socket.leave(roomName);
      logger.info(`Client socket ${socket.id} unsubscribed from updates for computer ${computerId} (left room ${roomName})`);
    } catch (error) {
      logger.error(`Error unsubscribing client ${socket.id} from computer ${computerId}: ${error.message}`, { stack: error.stack });
    }
  }
