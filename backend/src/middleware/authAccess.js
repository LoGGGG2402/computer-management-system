  ...
  return async (req, res, next) => {
    try {
      ...
      if (options.requiredRole === "admin" && !isAdmin) {
        logger.warn(`Access denied: Admin role required for user ${userId}`, {
          userId,
          role: userRole,
          endpoint,
          ip: req.ip,
        ...
        return res
          .status(403)
          .json({ status: "error", message: "Require Admin Role!" });
      }
      ...
      if (options.checkRoomIdParam) {
        ...
        if (!roomId) {
          logger.debug("Room access check failed: Missing room ID parameter", {
            endpoint,
          });
          return res
            .status(400)
        ...
        const hasAccess = await UserRoomAssignment.findOne({
          where: { user_id: userId, room_id: roomId },
        });
        if (!hasAccess) {
          logger.warn(`Access denied to room ${roomId} for user ${userId}`, {
            userId,
            roomId,
            role: userRole,
            endpoint,
        ...
        logger.debug(`Access granted to room ${roomId} for user ${userId}`, {
          endpoint,
        });
      ...
      if (options.checkComputerIdParam) {
        ...
        if (!computerId) {
          logger.debug(
            "Computer access check failed: Missing computer ID parameter",
            { endpoint }
          );
          return res
            .status(400)
            .json({ status: "error", message: "Computer ID is required" });
        }

        const hasAccess = await computerService.checkUserComputerAccess(
          userId,
          computerId
        );
        if (!hasAccess) {
          logger.warn(
            `Access denied to computer ${computerId} for user ${userId}`,
            { userId, computerId, role: userRole, endpoint, ip: req.ip }
          );
          return res.status(403).json({
            status: "error",
        ...
        logger.debug(
          `Access granted to computer ${computerId} for user ${userId}`,
          { endpoint }
        );
      }
