...
const uploadFileMiddleware = (options = {}) => {
  ...
  try {
    ...
    logger.error(`Failed to create upload directory: ${destination}`, {
      error: err.message,
      stack: err.stack,
    });
    return (req, res, next) => {
      res.status(500).json({
        status: "error",
        message: "Could not initialize upload directory.",
      });
    };
  }
  ...
  const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      logger.debug(`Generating filename for upload: ${newFilename}`, {
  ...
  const fileFilter = (req, file, cb) => {
    if (!allowedTypes || allowedTypes.length === 0) {
      logger.debug(
        `File type check skipped for ${file.originalname} (all types allowed)`
      );
      return cb(null, true);
    }
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      logger.warn(
        `Invalid file type rejected: ${file.mimetype} for ${file.originalname}`,
        {
          allowedTypes,
  ...
  return (req, res, next) => {
    singleUploadMiddleware(req, res, (err) => {
      if (err) {
        if (err instanceof multer.MulterError) {
          ...
          if (err.code === "LIMIT_UNEXPECTED_FILE") {
            return res.status(400).json({
              status: "error",
              message: `Unexpected file field '${err.field}'. Expected a single file using field name '${fieldName}'.`,
            });
          }
          return res
            .status(400)
      ...
      if (req.file) {
        ...
        logger.debug("No file was uploaded for the request.", {
          endpoint: `${req.method} ${req.originalUrl}`,
          ip: req.ip,
        });
      }
...
const agentPackageStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const version = req.body.version
      ? req.body.version.replace(/[^a-zA-Z0-9.-]/g, "_")
      : "";
...
const uploadAgentPackage = (req, res, next) => {
  ...
  uploadMiddleware(req, res, (err) => {
    ...
    logger.info(
      `Agent package uploaded successfully: ${req.file.filename} (${req.file.size} bytes)`
    );
    next();
  });
};
